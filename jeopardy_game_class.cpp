...
JeopardyGame::JeopardyGame(string fileName)
{
    ...
    if(fileName.empty())
        throw "Error: Cannot open empty fileName.";
    this->m_file_name = fileName;
    this->readFile();
    this->m_last_message = CHAR_ESC+"[1;32m[ MESSAGE ]"+CHAR_ESC+"[0m - Welcome to Jeopady! Select a question by typing category and number (ex. A2)";
    this->runGame();
...
void JeopardyGame::runGame()
{
    ...
    while(this->m_total_tries < totalAvailableTries && this->m_run_game)
    {
        ...
        {
            this->m_last_message = " "+CHAR_ESC+"[1;32m[ MESSAGE ]"+CHAR_ESC+"[0m  : That's correct! What's your next choice:";
            this->m_total_score += returnVal;
            this->m_total_tries++;
        }
        else if(returnVal == -1) // Incorrect answer - increment m_total_tries;
        {
            this->m_last_message = " "+CHAR_ESC+"[1;33m[ MESSAGE ]"+CHAR_ESC+"[0m : That's incorrect! Sorry :( What's your next choice:";
            this->m_total_tries++;
        }
        else if(returnVal == -2) // Not a valid input
        {
            this->m_last_message = " "+CHAR_ESC+"[1;31m[ MESSAGE ]"+CHAR_ESC+"[0m : Need a valid input.. Please try again:";
        }
        else if(returnVal == -3) // Already tried question
        {
            this->m_last_message = " "+CHAR_ESC+"[1;33m[ MESSAGE ]"+CHAR_ESC+"[0m : Already tried that one.. Please try another:";
        }
    }
    system(CLEAR_S);
...
void JeopardyGame::showEndScreen() const
{
    ...
    t.add(spacer + "Final Score:" + spacer);
    t.add(spacer + to_string(this->getScore()) + spacer);
    t.endOfRow();
    t.add(spacer + "Number Tries:" + spacer);
    t.add(spacer + to_string(this->getTries()) + spacer);
    t.endOfRow();
    t.setAlignment( 2, TextTable::Alignment::LEFT );
    cout << endl;
    cout << t;
    cout << endl << endl << CHAR_ESC+"[1;45m" << spacer2 << "GOOD JOB! Thanks for playing :)" << spacer2 << CHAR_ESC+"[0m\n\n\n";
}
...
void JeopardyGame::readFile()
{
    ...
    if(this->m_game_file.fail())
        throw "Error: Failed to open file: " + this->m_file_name;
    ...
    while(!this->m_game_file.eof()) // Read file and populate the m_num_questions_per_cat and m_num_answers_per_ques so that can dynamically allocate mem
    {
        ...
        if(numOfCategories > 7) // Limitation due to category m_cat_letters only going from A to G because H is used as help key
            throw "Error: Game only supports 7 categories. Please reduce number of categories and try again.";
    }
    ...
    while(!this->m_game_file.eof()) // Read file to populate question objects with the proper data
    {
        ...
        if(currGameElement > this->m_num_questions_per_cat) // If number of questions per category exceeds the amount specified (possible memory overflow/corruption)
            throw "Error: There's a problem with the jeopardy text file. You exceeded the amount of questions per category that you specified. Please fix and try again.";
        if(currPossibleAnswersCount > this->m_num_answers_per_ques)
            throw "Error: There's a problem with the jeopardy text file. You exceeded the amount of answers per question that you specified. Please fix and try again.";
    }
}
...
void JeopardyGame::showBoard() const
{
    ...
    */
    ...
    TextTable t( '~', '|', '+' );
    t.add("Score:" + to_string(this->getScore())); // Display score on top of table
    t.add(" ");
    t.add("Tries Remain: " + to_string(totalAvailableTries - this->getTries())); // Display tries remaining on top of table
    ...
    t.endOfRow();
    t.add(small_spacer + "Categories:" + small_spacer);
...
void JeopardyGame::showHelp() const
{
    ...
    this->showTitle();
    TextTable t( '-', '|', '+' );
    t.add(" Selecting a question: ");
    t.add(" Use the letter and number value for the question you are wanting to select (ex: B3). ");
    t.endOfRow();
    t.add(" Retries: ");
    t.add(" You are only allowed one try per question so choose wisely before selecting. ");
    t.endOfRow();
    t.add(" Points addition/deduction: ");
    t.add(" You get points added for correct answers but do not lost any points for incorrect ones. ");
    t.endOfRow();
    t.setAlignment( 2, TextTable::Alignment::LEFT );
    cout << endl;
    cout << t;
    cout << endl << endl << CHAR_ESC+"[1;34mEnter any key and press enter to go back.. "+CHAR_ESC+"[0m" << endl;
    cin >> anyInp;
}
...
void JeopardyGame::displayQuestion(int cat, int ques, vector<string> &answers) const
{
    const string small_spacer = "        ";
    TextTable t( '-', '|', '+' );
    t.add("##  Jeopardy Question: " + small_spacer); // Display the question itself
    t.endOfRow();
    t.add(small_spacer + this->m_game_elements[cat][ques].getQuestion() + small_spacer);
    t.endOfRow();
    t.add(" ");
    t.endOfRow();
    t.add("##  Possible Answers:" + small_spacer);
    t.endOfRow();
    ...
    cout << endl;
    cout << t;
    cout << endl << CHAR_ESC+"[1;34mWhats your answer:"+CHAR_ESC+"[0m ";
}
